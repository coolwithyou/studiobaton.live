...
export default function SidemenuPage() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      const [sectionsRes, categoriesRes] = await Promise.all([
        fetch("/api/admin/sidemenu/sections"),
        fetch("/api/admin/posts/categories"),
      ]);
  ...
  const handleDeleteSection = async (id: string) => {
    try {
      const response = await fetch(`/api/admin/sidemenu/sections?id=${id}`, {
        method: "DELETE",
      });
  ...
  const handleReorderSections = async (
    ...
    try {
      const response = await fetch("/api/admin/sidemenu/reorder", {
        method: "PATCH",
  ...
  const handleDeleteItem = async (id: string) => {
    try {
      const response = await fetch(`/api/admin/sidemenu/items?id=${id}`, {
        method: "DELETE",
      });
